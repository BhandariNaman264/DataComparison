    ...
    public class PSRTask : Task
    {
        ...
        public string ParamXml(ControlDB controlDB)
        {
            var jobParam = new Dictionary<string, object>()
                            {
                                { "NumPayPeriodsBeforeToday", WeeksBeforeToday },
                                { "NumPayPeriodsAfterToday", WeeksAfterToday },
                                { "RecalculateTransmittedPeriods", true },
        ...
        //public int GetClientId(ControlDB controlDB)
        //{
        ...
        */
        ...
        /*
        public string GetSiteIdsByNames(ControlDB controlDB, string defaultValue = "")
        {
            if (String.IsNullOrEmpty(OrgUnit))
                return defaultValue;

            string sql = $@"SELECT OrgUnitId FROM OrgUnit WITH(NOLOCK) WHERE ShortName IN ({GetSiteNames(OrgUnit)})";
        ...
        */
        ...
        public string GetJobSql(ControlDB controlDB)
        {
            string insertSql = $@"
                ...
                (
                    GETUTCDATE(),
                    '{Namespace}',
                    'SharpTop.Engine.BackgroundJobs.PaySummaryRecalc',
                    {GetJobIdForClient(controlDB, "PaySummaryRecalc")},
                    NEWID(),
                    'Pay Summary Recalc',
                    'BackgroundJobs.dll',
                    'SharpTop.Engine.BackgroundJobs.PaySummaryRecalc',
                    ...
                    '{ParamXml(controlDB)}',
                    '{CONTEXTXML}',
                    {GetJobDefinitionId(controlDB, "PaySummaryRecalc")},
                    'PaySummaryRecalc',
        ...
        public string GetFetchRecordSql(ControlDB controlDB)
        {
            ...
            string sql = $@"
                            ...
                            FROM
                                EmployeePaySummary eps WITH(NOLOCK)
                            WHERE
        ...
        public bool FetchRecords(ControlDB controlDB, string pairName, bool forceCompareOnly, string outputFolder)
        {
            ...
            _m_Log.AppendLine($"Fetching pay summaries for the instance '{Namespace}' from '{FromDate}' to '{ToDate}'...");
            ...
            catch (Exception e)
            {
                ...
                _m_Log.AppendLine($"FAILED to fetch pay summaries.  Error:");
        ...
        private bool SaveRecordsToFile(DataTable dt, string pairName, string outputFolder)
        {
            ...
            catch (Exception e)
            {
                ...
                _m_Log.AppendLine($@"FAILED to save pay summary records to file for instance '{Namespace}'.  Error:");
                _m_Log.AppendLine(e.Message);
                return false;
            }
        }
...
";
        ...
        private string BuildRecord(DataRow row, ref long totalMinutes, ref decimal totalHours, ref decimal totalAmount)
        {
            ...
            if (row["PayGroupCalendarId"] != DBNull.Value)
                sb.Append($"{Convert.ToInt64(row["PayGroupCalendarId"])}");
        ...
        public bool CompareFiles(PSRTask task2, string pairName, string outputFolder)
        {
